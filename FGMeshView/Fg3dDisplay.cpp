...
FGLINK(linkRenderOpts)
{
    ...
    FgVect3F            bcolor = inputs[8]->valueRef();
    ...
    uint                        simple)
{
    ...
    ret.morphCtls =
        fgGuiSplitScroll(g_gg.depStub(morphNamesN.idx()),boost::bind(getPanes,morphNamesN,morphValsN));
    ...
    g_gg.addLink(linkNorms2,fgUints(meshesN,ret.morphedVertssN),fgUints(api.normssN));
    FgDgn<bool>         useTexture = g_gg.addInput(true,uid+"UseTexture"),
    ...
    g_gg.addLink(linkRenderOpts,
        fgUints(facets,useTexture,shiny,wireframe,flatShaded,surfPoints,markedVerts,allVerts,bgColorN),
    ...
    FgGuiPtr            rcColor = fgGuiCheckboxTr("Colour maps",useTexture),
    ...
    if (simple == 0)
        renderCtls = fgGuiSplit(true,
            fgGuiSplit(false,rcColor,rcShiny,rcWire,rcFlat),
            fgGuiSplit(false,rcFacet,rcSurf,rcMark,rcAll));
    else if (simple == 1)
...
fgDisplayMeshes(
    ...
    bool                            compare)
{
	TRACE("In fgDisplayMeshes..., compare is %s\n", (compare ? "true" : "false"));
    ...
    FgMatrix32F                 viewBounds = fgBounds(meshes[0].verts);
    for (size_t ii=1; ii<meshes.size(); ++ii)
        viewBounds = fgBounds(viewBounds,fgBounds(meshes[ii].verts));
    FgDgn<FgMatrix32D>          viewBoundsN = g_gg.addNode(fgF2D(viewBounds),"viewBounds");
...
fgDisplayForEdit(const Fg3dMesh & mesh)
{
    ...
    FgGuiTab                    meshSelect;
    FgDgn<FgMatrix32D>          viewBoundsN = g_gg.addNode(fgF2D(fgBounds(mesh.verts)),"viewBounds");
    FgDgn<FgString>             meshStatsN(g_gg,"meshStats");
    g_gg.addLink(linkMeshStats2,fgUints(meshesN),fgUints(meshStatsN));
    FgGui3dCtls         ga3 = fgGui3dCtls(meshesN,allVertsN,texssN,viewBoundsN);
    vector<FgGuiTab>    tabDefs = fgSvec(
