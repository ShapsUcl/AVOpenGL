...
struct  FgGuiWin3dOgl : public FgGuiOsBase
{
    ...
    updateIfChanged()
    {
        if (m_api.changed()) {
            ...
            // actually send a WM_PAINT message until the message queue is empty. If the
            // window is not updated that means the message queue is getting flooded.
    ...
    render()
    {
        if (g_gg.changed(m_api.texssN))     // Always true on first run-through
            loadImages();
        if (g_gg.changed(m_api.light))
        ...
        fgOglRender(
            rendModels,
            FgMatrix44F(camera.modelview.asHomogenous().transpose()),
            camera.frustum,
            render);
    }
};
